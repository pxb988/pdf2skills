---
name: pdf2skills
description: Use when a user wants to convert a PDF or Markdown document into a structured skill pack for an agent runtime, especially when they want reusable, callable knowledge extracted from manuals, books, reports, standards, or project documents.
---

# PDF / Markdown to Skills Pipeline

Transform PDF or Markdown documents into structured, callable skills through a multi-stage pipeline that combines Python NLP computation with AI semantic analysis.

## How It Works

The pipeline reads a PDF, breaks it into semantically coherent chunks, identifies high-value knowledge regions, extracts structured knowledge units, deduplicates them, and converts them into skill format.
...
PDF parsing defaults to the runtime's built-in document reading capability (zero config). For better quality on complex PDFs, configure an external LLM provider:
...
5. **If the input is a PDF:**
   - Get the page count:
     ...
     ```
   - Read the PDF page by page using the runtime's document reading tool with the `pages` parameter (max 20 pages per call). Combine all content into a single Markdown document.
...
Read `{project_root}/skills/pdf2skills/prompts/skill-generation.md`. For each bucket in `{output_dir}/skus/buckets.json`, dispatch an Agent to convert the SKUs into structured skills.
...
2. **Install**: "Copy skill folders into a skill directory recognized by your target agent runtime"
3. **Use**: "Load or invoke each skill according to that runtime's normal mechanism"
...
For very large PDFs (>100 pages), suggest splitting into parts first. The runtime's document reading tool handles max 20 pages per call, so a 200-page book needs 10 read operations.
